...
public class Asset {
    ...
    public boolean equals(final Object obj) {
        ...
        if ((obj != null) && (obj instanceof AssetField)) {
            ...
            if (asset.issueKey.equals(issueKey)
                    && asset.deviceName.equals(deviceName)
                    && asset.assignee.getName().equals(assignee.getName())
                    && asset.reporter.getName().equals(reporter.getName())
                    && asset.status.equals(status)
                    && asset.assetDetails.equals(assetDetails)
                    && Boolean.toString(asset.loggedUserIsAssignee).equals(Boolean.toString(loggedUserIsAssignee))
                    && Boolean.toString(asset.loggedUserIsPreviousAssigned).equals(
                            Boolean.toString(loggedUserIsPreviousAssigned))
                    && Boolean.toString(asset.loggedUserIsReporter).equals(Boolean.toString(loggedUserIsReporter))) {
                result = true;
            }
        }
        return result;
