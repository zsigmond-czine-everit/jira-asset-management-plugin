...
public class JiraPluginServiceImpl implements JiraPluginService {
    ...
    public boolean changeIssueStatus(final String issueKey, final String issueAssignee, final String actionName,
        ...
        User actUser = cm.getUserUtil().getUserObject(actualUserName);
        User loggedUser = getLoggedUser();
        if (loggedUser.getName().equals(actualUserName)) {
            if ((actUser != null) && permissionCheck(Permissions.EDIT_ISSUE, actUser)) {
                User assigneeUser = ComponentAccessor.getUserManager().getUserObject(issueAssignee);
        ...
        if (!assetDetails.isEmpty() && !issues.isEmpty()) {
            for (AssetDetail ad : assetDetails) {
                ...
                if ((issue != null) && (loggedUser != null)) {
                    ...
                    asset.setAssignee(ComponentAccessor.getUserManager().getUserObject(issue.getString("assignee")));
                    ...
                    asset.setReporter(ComponentAccessor.getUserManager().getUserObject(issue.getString("reporter")));
                    ...
                    if ((previousAssignedUser != null) && previousAssignedUser.getName().equals(loggedUser.getName())) {
    ...
    public boolean isValidAssignee(final String assigneeName) {
        ...
        User user = ComponentManager.getInstance().getUserUtil().getUserObject(assigneeName);
        if (user != null) {
            return true;
        }
        return false;
            ...
            if ((allChangeItems != null) && !allChangeItems.isEmpty()) {
                ...
                for (int i = size; i >= 0; i--) {
                    ...
                    if (chi.getField().equals("assignee") && isValidAssignee(chi.getFromValue())) {
                        oldUserName = ComponentAccessor.getUserManager().getUserObject(chi.getFromValue());
                        break;
                    }
                }
            }
